...
typedef struct _MV_CC_ROTATE_IMAGE_PARAM_T_ {
  enum MvGvspPixelType enPixelType; ///< [IN]  \~chinese 像素格式 \~english
                                    ///< Pixel format
  unsigned int
      nWidth; ///< [IN][OUT] \~chinese 图像宽             \~english Width
  unsigned int
      nHeight; ///< [IN][OUT] \~chinese 图像高             \~english Height

  unsigned char *pSrcData; ///< [IN]  \~chinese 输入数据缓存           \~english
                           ///< Input data buffer
  unsigned int nSrcDataLen; ///< [IN]  \~chinese 输入数据长度 \~english Input
                            ///< data length

  unsigned char *pDstBuf; ///< [OUT] \~chinese 输出数据缓存           \~english
                          ///< Output data buffer
  unsigned int nDstBufLen; ///< [OUT] \~chinese 输出数据长度           \~english
                           ///< Output data length
  unsigned int nDstBufSize; ///< [IN]  \~chinese 提供的输出缓冲区大小 \~english
                            ///< Provided output buffer size

  MV_IMG_ROTATION_ANGLE enRotationAngle; ///< [IN]  \~chinese 旋转角度 \~english
                                         ///< Rotation angle

  unsigned int
      nRes[8]; ///<       \~chinese 预留                   \~english Reserved
...
typedef struct _MV_CC_FLIP_IMAGE_PARAM_T_ {
  ...
  unsigned int
      nHeight; ///< [IN]  \~chinese 图像高                 \~english Height
...
typedef enum _MV_CC_GAMMA_TYPE_ {
  ...
  ///< \~chinese 长度：256*sizeof(unsigned char)      \~english
  ///< 8bit,length:256*sizeof(unsigned char)
  MV_CC_GAMMA_TYPE_LRGB2SRGB = 3, ///< \~chinese linear RGB to sRGB \~english
                                  ///< linear RGB to sRGB
  MV_CC_GAMMA_TYPE_SRGB2LRGB =
      4, ///< \~chinese sRGB to linear RGB(仅色彩插值时支持，色彩校正时无效)
         ///< \~english sRGB to linear RGB
...
typedef struct _MV_CC_GAMMA_PARAM_T_ {
  MV_CC_GAMMA_TYPE enGammaType; ///< [IN]  \~chinese Gamma类型 \~english Gamma
                                ///< type
  float fGammaValue; ///< [IN]  \~chinese Gamma值[0.1,4.0]       \~english Gamma
                     ///< value[0.1,4.0]
  unsigned char *pGammaCurveBuf; ///< [IN]  \~chinese Gamma曲线缓存 \~english
                                 ///< Gamma curve buffer
  unsigned int nGammaCurveBufLen; ///< [IN]  \~chinese Gamma曲线长度 \~english
                                  ///< Gamma curve buffer size
...
typedef struct _MV_CC_FRAME_SPEC_INFO_ {
  ...
  unsigned int nCycleCount; ///< [OUT] \~chinese 周期数 \~english The Count of
                            ///< Cycle
  unsigned int nCycleOffset; ///< [OUT] \~chinese 周期偏移量 \~english The
                             ///< Offset of Cycle

  float fGain; ///< [OUT] \~chinese 增益                   \~english Gain
  float fExposureTime; ///< [OUT] \~chinese 曝光时间               \~english
                       ///< Exposure Time
  unsigned int nAverageBrightness; ///< [OUT] \~chinese 平均亮度 \~english
                                   ///< Average brightness

  /// \~chinese 白平衡相关        \~english White balance
  unsigned int nRed; ///< [OUT] \~chinese 红色                   \~english Red
  unsigned int
      nGreen; ///< [OUT] \~chinese 绿色                   \~english Green
  unsigned int nBlue; ///< [OUT] \~chinese 蓝色                   \~english Blue

  unsigned int nFrameCounter; ///< [OUT] \~chinese 总帧数 \~english Frame
                              ///< Counter
  unsigned int nTriggerIndex; ///< [OUT] \~chinese 触发计数 \~english Trigger
                              ///< Counting

  unsigned int
      nInput; ///< [OUT] \~chinese 输入                   \~english Input
  unsigned int
      nOutput; ///< [OUT] \~chinese 输出                   \~english Output

  /// \~chinese ROI区域           \~english ROI Region
  unsigned short
      nOffsetX; ///< [OUT] \~chinese 水平偏移量             \~english OffsetX
  unsigned short
      nOffsetY; ///< [OUT] \~chinese 垂直偏移量             \~english OffsetY
  unsigned short nFrameWidth; ///< [OUT] \~chinese 水印宽 \~english The Width of
                              ///< Chunk
  unsigned short nFrameHeight; ///< [OUT] \~chinese 水印高 \~english The Height
                               ///< of Chunk
...
/// \~chinese 无损解码参数              \~english High Bandwidth decode
/// structure
typedef struct _MV_CC_HB_DECODE_PARAM_T_ {
  unsigned char *pSrcBuf; ///< [IN]  \~chinese 输入数据缓存           \~english
                          ///< Input data buffer
  unsigned int nSrcLen; ///< [IN]  \~chinese 输入数据大小           \~english
                        ///< Input data size

  unsigned int
      nWidth; ///< [OUT] \~chinese 图像宽                 \~english Width
  unsigned int
      nHeight; ///< [OUT] \~chinese 图像高                 \~english Height
  unsigned char *pDstBuf; ///< [OUT] \~chinese 输出数据缓存           \~english
                          ///< Output data buffer
  unsigned int nDstBufSize; ///< [IN]  \~chinese 提供的输出缓冲区大小 \~english
                            ///< Provided output buffer size
  unsigned int nDstBufLen; ///< [OUT] \~chinese 输出数据大小           \~english
                           ///< Output data size
  enum MvGvspPixelType enDstPixelType; ///< [OUT] \~chinese 输出的像素格式
                                       ///< \~english Output pixel format

  MV_CC_FRAME_SPEC_INFO stFrameSpecInfo; ///< [OUT] \~chinese 水印信息 \~english
                                         ///< Frame Spec Info
...
typedef struct _MV_ACTION_CMD_INFO_T {
  ...
  unsigned int
      bSpecialNetEnable; ///< [IN]  \~chinese
                         ///< 只有设置成1时指定的网卡IP才有效，非1时无效
                         ///< \~english Special IP Enable
  unsigned int nSpecialNetIP; ///< [IN]  \~chinese 指定的网卡IP \~english
                              ///< Special Net IP address
